...
- **强制只读**：在`@virid/vue`中，只读不仅仅只是建议，通过运行时 `borrowChecker` 机制，**所有非自身所有**的数据，都被强制转化为 **“深度只读”** 禁止任何写操作，甚至连方法也无法任意调用，除非被@Safe装饰器标记。在物理层面杜绝了 UI 组件意外污染非自身的可能性。
...
## 🛡️ 运行时借用检查 (Borrow Checker)

在 `@virid/vue` 中，**“禁止修改父组件数据”不是一种建议，而是一种铁律。**

为了确保确定性，所有通过 `@Project` 或 `@Inherit` 获取的外部数据，都会被自动套上一层递归的运行时借用检查。 该检查器会拦截所有的 **写操作 (Set)** 以及 **非法方法调用**并**详细指出原因及其访问的路径和错误调用栈**。

### 1. 拦截行为

- **赋值拦截**：尝试修改对象的属性将直接触发异常。
- **变异方法拦截**：禁止调用任何可能修改原数据的函数（如 `Array.push`, `Map.set`, `Set.add`）。
- **深度递归**：检查器是递归生效的且惰性缓存的，无论数据嵌套多深，其后代节点均受保护，且只有访问时才生效。
